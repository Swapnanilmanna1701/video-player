...
export function getVideosByCategory(categorySlug: string): Video[] {
  return getAllVideos().filter((v) => v.categorySlug === categorySlug);
}

export function getNextVideo(currentVideo: Video): Video | null {
  const categoryVideos = getVideosByCategory(currentVideo.categorySlug);
  const currentIndex = categoryVideos.findIndex((v) => v.slug === currentVideo.slug);
